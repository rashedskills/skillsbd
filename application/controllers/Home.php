...
class Home extends CI_Controller {
    ...
    public function courses() {
        ...
        if (isset($_GET['course_type']) && !empty($_GET['course_type'])) {
            $selected_type = $_GET['course_type'];
        }
        ...
        if ($selected_category_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_type == 'all' && $selected_language == 'all' && $selected_rating == 'all') {
            ...
            $courses = $this->crud_model->filter_course($selected_category_id, $selected_price, $selected_level, $selected_type, $selected_language, $selected_rating);
    ...
    public function my_refund() {
        ...
        $page_data['page_name'] = "my_refund";
        $page_data['page_title'] = get_phrase("my_refund");
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function order_history() {
        ...
        $total_rows = $this->crud_model->order_history($this->session->userdata('user_id'))->num_rows();
        $config = array();
        $config = pagintaion($total_rows, 10);
        $config['base_url']  = site_url('home/order_history');
        $this->pagination->initialize($config);
        $page_data['per_page']   = $config['per_page'];
        $page_data['page_name']  = "order_history";
        $page_data['page_title'] = get_phrase('order_history');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }

    public function remove_order($order_id = ""){
        $this->crud_model->delete_order($order_id);
        $this->session->set_flashdata('flash_message', get_phrase('order_deleted'));
    ...
    public function ssl_checkout() {
        if ($this->session->userdata('user_login') != 1)
        redirect('home', 'refresh');

        $total_price_of_checking_out  = $_SESSION['tprice'];
        ...
        $this->load->view('frontend/'.get_frontend_settings('theme').'/ssl_checkout', $page_data);
    }
    ...
    public function easycheckout_endpoint()
    {
        ...
        $post_data = array();
        $post_data['total_amount'] = $_SESSION['tprice'];
        ...
        $post_data['cus_name'] = $_SESSION['fname'].' '.$_SESSION['lname'];
        $post_data['cus_email'] = $_SESSION['uemail'];
        ...
        $post_data['product_profile'] = "course";
    ...
    public function success_payment()
    {
        ...
        $database_order_status = 'success'; // Check this from your database here Pending is dummy data,
    ...
    {
        $this->session->set_flashdata('error_message', get_phrase('course_purchase_failed!'));
        redirect('home/ssl_checkout', 'refresh');  
    }
    public function cancel_payment()
    {
        $this->session->set_flashdata('error_message', get_phrase('course_purchase_cancel!'));
        redirect('home/ssl_checkout', 'refresh'); 
    }
    ...
    public function become_instructor() {
        $page_data['page_name'] = 'become_instructor';
        $page_data['page_title'] = get_phrase('become_instructor');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }

    public function providers() {
        $page_data['page_name'] = 'institute_providers';
        $page_data['page_title'] = get_phrase('advertise_a_course');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function instructor_terms() {
        $page_data['page_name'] = 'instructor_terms';
        $page_data['page_title'] = get_phrase('instructor_terms_of_use');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function get_enrolled_to_classroom_course($course_id) {
        if ($this->session->userdata('user_login') == 1) {
            $this->crud_model->enrol_to_classroom_course($course_id, $this->session->userdata('user_id'));
            redirect(site_url('home/my_courses'), 'refresh');
        }else {
            redirect(site_url('login'), 'refresh');
        }
