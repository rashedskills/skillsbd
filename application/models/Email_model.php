...
class Email_model extends CI_Model {
	...
	public function send_purchase_notification_mail($cart_items = ""){
		$websiteUrl = 'http://skillsbd.com';
		...
		$purchase_msg	.=	"<strong style='padding: 10px 0px; font-size: 14px; text-decoration: underline'>Purchased Course:</strong><br><br>";
		foreach ($this->session->userdata('cart_items') as $cart_items) {
		$course_details  = $this->crud_model->get_course_by_id($cart_items)->row_array();
		$purchase_msg	.= "<span style='font-size: 20px;'>&#8226;</span><strong style='font-size: 14px;'> ".$course_details['title']."</strong><br>";
		}
		$purchase_msg 	.= 	"<strong style='padding: 10px 0px; text-decoration: underline; font-size: 14px;'>Customer Details:</strong><br><br>";
		$purchase_msg 	.= 	"<strong style='padding-bottom: 10px;'>Phone: </strong>".$user_details["phone"];
		$purchase_msg 	.= 	"<hr>";
