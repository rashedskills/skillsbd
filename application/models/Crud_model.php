...
class Crud_model extends CI_Model {
  ...
  public function order_history($user_id) {
    if ($user_id > 0) {
      return $this->db->get_where('order_new', array('user_id'=> $user_id));
    }else {
  ...
  public function get_all_instructors(){
      ...
      return $this->db->get('my_instructors');
  }

  public function get_instructors_detail_by_id($instructor_id = ""){
    return $this->db->get_where('my_instructors', array('id' => $instructor_id))->row_array();
  }
  ...
  public function add_course($param1 = "") {
    ...
    $schedule = $this->trim_and_return_json($this->input->post('course_schedule'));
    ...
    $data['course_schedule'] = $schedule;
    $data['duration'] = $this->input->post('duration');
    $data['total_classes'] = $this->input->post('total_classes');
    $data['total_hours'] = $this->input->post('total_hours');
    $data['venue'] = $this->input->post('venue');
    $data['vat_included'] = $this->input->post('vat_included');
    $data['my_instructor_id'] = $this->input->post('my_instructor_id');
  ...
  public function update_course($course_id, $type = "") {
    ...
    $data['end_date'] = $this->input->post('end_date');
    $data['reg_last_date'] = $this->input->post('reg_last_date');
    $data['course_schedule'] = $this->trim_and_return_json($this->input->post('course_schedule'));
    $data['duration'] = $this->input->post('duration');
    $data['total_classes'] = $this->input->post('total_classes');
    $data['total_hours'] = $this->input->post('total_hours');
    $data['venue'] = $this->input->post('venue');
  ...
  public function get_order_id($order_id = "") {
    return $this->db->get_where('order_new', array('id' => $order_id));
  }
  ...
  public function get_classroom_courses() {
    return $this->db->get_where('course', array('course_schedule' => 'Workshop', 'course_type' => 'Classroom', 'start_date >' => '12/12/2019', 'status' => 'active'));
    //return $this->db->get_where('course', array('is_top_course' => 1, 'course_type' => 'Online', 'status' => 'active'));
  }
  ...
  public function update_order($order_id){
    ...
    $this->db->where('id', $order_id);
    $this->db->update('order_new', $statusUpdate);
  }
  ...
  public function add_lesson() {
    ...
    if($lesson_type == 'video') {
      ...
      if ($this->input->post('html5_video_url_for_mobile_application') == "" || $this->input->post('html5_duration_for_mobile_application') == "") {
        $this->session->set_flashdata('error_message',get_phrase('invalid_lesson_url_and_duration_for_mobile_application'));
        redirect(site_url(strtolower($this->session->userdata('role')).'/course_form/course_edit/'.$data['course_id']), 'refresh');
      }
      $duration_for_mobile_application_formatter = explode(':', $this->input->post('html5_duration_for_mobile_application'));
      ...
      $data['video_url_for_mobile_application'] = $this->input->post('html5_video_url_for_mobile_application');
  ...
  public function enrol_to_classroom_course($course_id = "", $user_id = "") {
    $course_details = $this->get_course_by_id($course_id)->row_array();
    if ($course_details['course_type'] == 'Classroom' || $course_details['course_type'] == 'Workshop') {
      $data['course_id'] = $course_id;
      $data['user_id']   = $user_id;
      if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
        $this->session->set_flashdata('error_message', get_phrase('student_has_already_been_enrolled_to_this_course'));
  ...
  function curl_request($code = '') {
    ...
    // version 1.4
    function filter_course($selected_category_id = "", $selected_price = "", $selected_level = "", $selected_type = "", $selected_language = "", $selected_rating = ""){
     //echo $selected_category_id.' '.$selected_price.' '.$selected_level.' '.$selected_type.' '.$selected_language.' '.$selected_rating;
      ...
      if ($selected_type != "all") {
        $this->db->where('course_type', $selected_type);
      }
